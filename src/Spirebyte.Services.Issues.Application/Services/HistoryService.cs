            ...
            switch (fieldType)
            {
                ...
                case FieldTypes.LinkedIssues:
                    var oldPropertyArray =
                        ((IEnumerable<Guid>)oldProperty.GetValue(oldIssue) ?? Enumerable.Empty<Guid>()).ToArray();
                    var newPropertyArray =
                        ((IEnumerable<Guid>)matchingProperty.GetValue(newIssue) ?? Enumerable.Empty<Guid>()).ToArray();
